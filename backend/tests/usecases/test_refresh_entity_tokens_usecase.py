...
class FakeTokenService:
    ...
    async def issue_refresh_token(
        ...
        *,
        ...
        return f"refresh::{session_id}::{entity_id}::{token_version}::{self._refresh_counter}"
...
@pytest.mark.asyncio
async def test_refresh_entity_tokens_usecase_rotates_tokens() -> None:
    """验证刷新成功时会消费旧 refresh 并返回新 token 对。"""
    session_repo = InMemorySessionRepository()
    await session_repo.create(session_id="session_demo", description=None, max_entities_limit=10)
    ...
    usecase = RefreshEntityTokensUseCase(session_repo, profile_repo, auth_repo, token_service)
    ...
    assert result.access_token.startswith("access::session_demo::entity_1::1")
    assert result.refresh_token.startswith("refresh::session_demo::entity_1::1")
    assert result.access_token_expires_in == 900
    assert result.refresh_token_expires_in == 604800


@pytest.mark.asyncio
async def test_refresh_entity_tokens_usecase_revokes_all_when_replay_detected() -> None:
