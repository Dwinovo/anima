...
class InMemoryProfileRepository:
    ...
    async def save(
        ...
        *,
        ...
        self._existing_ids.add(entity_id)

    async def get(self, *, session_id: str, entity_id: str) -> str | None:
        """读取画像。"""
        _ = session_id
        if entity_id in self._existing_ids:
            return '{"name":"demo"}'
        return None
    ...
    async def delete(self, *, session_id: str, entity_id: str) -> None:
        """删除画像。"""
        _ = session_id
        self._existing_ids.discard(entity_id)
...
async def test_get_entity_context_usecase_returns_views_payload() -> None:
    ...
    presence_repo = InMemoryPresenceRepository(active_ids={"entity_me"})
    profile_repo = InMemoryProfileRepository(existing_ids={"entity_me"})
    ...
    usecase = GetEntityContextUseCase(
        session_repo,
        presence_repo,
        profile_repo,
        payload_repo,
...
@pytest.mark.asyncio
async def test_get_entity_context_usecase_sets_has_more_and_next_cursor() -> None:
    """验证事件类视图在超限时会返回分页标记。"""
    session_repo = InMemorySessionRepository()
    await session_repo.create(
        session_id="session_demo",
...
@pytest.mark.asyncio
async def test_get_entity_context_usecase_raises_when_session_missing() -> None:
    """验证 Session 不存在时会抛出异常。"""
    usecase = GetEntityContextUseCase(
        InMemorySessionRepository(),
        InMemoryPresenceRepository(active_ids={"entity_me"}),
        InMemoryProfileRepository(existing_ids={"entity_me"}),
        InMemoryPayloadRepository(payloads={}),
        InMemoryGraphRepository(event_ids=[]),
    )

    with pytest.raises(SessionNotFoundException):
        await usecase.execute(session_id="session_missing", entity_id="entity_me")


@pytest.mark.asyncio
async def test_get_entity_context_usecase_raises_when_entity_missing() -> None:
    """验证 Entity 不存在时会抛出异常。"""
    session_repo = InMemorySessionRepository()
    await session_repo.create(
        session_id="session_demo",
