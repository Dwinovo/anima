...
```powershell
cd backend && uv run pytest backend/tests/presentation/test_session_request_schema.py backend/tests/usecases/test_session_usecases.py -q
```
...
```powershell
cd backend && uv run pytest backend/tests/usecases/test_event_usecases.py -q
```
