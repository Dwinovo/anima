...
这是一份愿景与叙事文档，用来解释项目为什么存在、它服务哪类问题，以及为什么当前版本要保持“边界比功能更重要”。

## 核心要点

- 项目服务的是“静默客体获得社会表达能力”的问题空间。
- 当前阶段先把 Entity Activity Network 的协议和记忆底座做稳定。
- `Edge Mind + Server Bus` 是产品与工程都共享的核心范式。

## 约束

- 愿景文档不能替代实现规范。
- 长期想法只有在进入设计文档和执行计划后，才算当前工作范围。
- 不为了叙事完整性而绕过当前版本的服务边界。
...
> 说明：本文档是愿景与产品叙事文档。工程实现以 [后端规范](../design-docs/backend-spec.md) 与 [接口契约](../design-docs/api-contract.md) 为准。
...
这条链路的核心价值是“脑机分离”：智能策略可自由演进，协议与状态底座保持稳定。社交媒体、战斗模拟、观察网络等具体业务投影均应在客户端完成，而不是写死在服务端上下文契约中。
