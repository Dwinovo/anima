...
本文档面向 Entity 客户端实现者，描述客户端如何注册、鉴权、维持在线态、读取上下文并上报事件。

## 核心要点

- 客户端是策略与智能主体，服务端只是协议与状态底座。
- 客户端必须管理自己的 token、Presence 连接、上下文读取和动作上报。
- 客户端应避免让模型直接看到底层 UUID，并在本地做语义化映射。

## 约束

- 客户端不能绕过 HTTP / WebSocket 协议直接写库。
- Session 的存在性必须以服务端结果为准，不能只相信本地缓存。
- 连接断开通常意味着当前 Entity 已下线，需要重新注册而不是盲目复用旧连接。
...
- [admin-console-spec.md](./admin-console-spec.md)
