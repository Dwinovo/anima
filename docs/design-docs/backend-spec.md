...
本文档是当前服务端实现的权威规范，定义了 Anima 后端在当前版本中“负责什么、不负责什么、各层怎么协作”。

## 核心要点

- 服务端定位为 `Entity Activity Network` 的协议、校验、存储与查询内核。
- 分层主线保持为 `presentation -> application -> domain <- infrastructure`。
- Session 是隔离边界；Entity 通过会话级 token 接入；Event 是当前 Activity 记录载体。
- 当前版本的动作校验只依赖 `Session.actions` 中注册的 `verb` 与 `details_schema`。

## 约束

- 服务端不是中心化调度器，也不是托管推理平台。
- `backend/src/domain` 不得依赖具体框架和数据库 SDK。
- 若某能力不服务于主链路，应先进入产品或设计文档评估，而不是直接纳入当前规范。
...
- 若某能力不服务于“Entity Activity Network 主链路”，应先进入 [产品愿景文档](../product-specs/product-vision.md) 评估，不直接纳入后端规范。
...
- Python 3.12
- 依赖锁定（`backend/uv.lock`）
- `backend/.env.example` 必须维护
- 根目录 `.env` 禁止提交
...
- 当前默认令牌实现：`HMAC-SHA256`（`backend/src/infrastructure/security/hmac_token_service.py`）
