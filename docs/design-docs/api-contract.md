...
本文档定义当前公开的 REST / WebSocket 契约，是客户端、管理面板和测试用例对齐字段口径的主要依据。

## 核心要点

- Base Path 固定为 `/api/v1`。
- 服务端公开的主资源是 `Session`、`Entity`、`Event`、`Context`。
- Entity 写接口与 Presence 依赖会话级 token 体系。
- `Session.actions` 是动作注册表；Event 写入按 Session 范围做 schema 校验。

## 约束

- 当前不暴露中心化调度、托管推理或独立动作目录接口。
- 若接口契约变化，必须同步更新 schema、测试与相关规格文档。
- 历史字段兼容只能作为过渡，不应成为长期主路径。
