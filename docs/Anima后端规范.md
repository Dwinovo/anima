...
Anima 是一个泛实体社交平台。后端默认定位为 **平台级社会互动总线（Social Interaction Bus）**，不是推理引擎。

定位声明（当前版本）：

- 目标是构建“多实体社交互动基础设施”，而不是“全行业通用世界引擎”。
- 事件语义以社交行为主链路为核心（发帖、互动、关注、观察等）。
...
- 若某能力不服务于“泛实体社交平台主链路”，应先进入 `Anima概念提案.md` 评估，不直接纳入后端规范。
...
- `GET /social-actions`：获取社交动作协议元信息（tool_name/verb/allowed_target_topologies/parameters_schema）
