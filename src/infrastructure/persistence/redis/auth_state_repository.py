...
class RedisAuthStateRepository(AgentAuthStateRepository):
    """Redis 侧：Agent 鉴权状态仓储实现。"""
    ...
    async def ensure_token_version(
        ...
        *,
        ...
        key = auth_token_version_key(session_id, agent_id)
    ...
    async def get_token_version(self, *, session_id: str, agent_id: str) -> int | None:
        """读取 token_version。"""
        existing = await self._client.get_value(auth_token_version_key(session_id, agent_id))
        if existing is None:
            return None
        try:
            return int(existing)
    ...
    async def bump_token_version(self, *, session_id: str, agent_id: str) -> int:
        ...
        return await self._client.incr_value(auth_token_version_key(session_id, agent_id))
    ...
    async def store_refresh_jti(
        ...
        *,
        ...
        """存储 refresh_jti。"""
        token_key = auth_refresh_token_key(session_id, agent_id, refresh_jti)
        index_key = auth_refresh_index_key(session_id, agent_id)
    ...
    async def consume_refresh_jti(
        ...
        *,
        ...
        await self._client.remove_set_member(
            auth_refresh_index_key(session_id, agent_id),
            refresh_jti,
        )
        return value is not None

    async def revoke_all_refresh_jti(self, *, session_id: str, agent_id: str) -> None:
        """撤销指定 Agent 的全部 refresh_jti。"""
        index_key = auth_refresh_index_key(session_id, agent_id)
        jti_list = await self._client.get_set_members(index_key)
        keys = [auth_refresh_token_key(session_id, agent_id, refresh_jti) for refresh_jti in jti_list]
